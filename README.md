...
- **Google Maps (mapas visuales)** — `VITE_GOOGLE_MAPS_API_KEY` en `apps/admin/.env` (copia `apps/admin/.env.example`; habilita "Maps JavaScript API" en el mismo proyecto de Google Cloud). El Mapa de Leads y el mapa de calor de Reportes (`apps/admin/src/components/GeoMap.tsx`) usan mapas reales de Google en vez del lienzo de OpenStreetMap/Leaflet que traía el prototipo. Es una clave expuesta en el navegador (normal para este uso) — protégela restringiéndola por HTTP referrer en Google Cloud Console a los dominios donde corre el admin, no ocultándola. Sin esta variable, esas dos pantallas muestran un aviso en vez de un mapa en blanco.
