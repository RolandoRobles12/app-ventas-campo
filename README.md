...
- **Google Maps (mapas visuales)** — `VITE_GOOGLE_MAPS_API_KEY` en `apps/admin/.env` (copia `apps/admin/.env.example`; habilita **"Maps JavaScript API"** y **"Places API"** en el mismo proyecto de Google Cloud). Es una clave expuesta en el navegador (normal para este uso) — protégela restringiéndola por HTTP referrer en Google Cloud Console a los dominios donde corre el admin, no ocultándola. Sin esta variable, las pantallas de mapa muestran un aviso en vez de un mapa en blanco. Se usa en tres lugares:
